...
// Approachable, rounded, soft shadows, warm tones, emoji accents,
// conversational copy. Feels like a community bulletin board.

function WarmCard({ tournament }: { tournament: Tournament }) {
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        ...
        <span title={tournament.registration_status ?? "open"}>
          {statusEmoji[tournament.registration_status ?? "open"] ?? "🟢"}
        </span>
      </div>
      ...
      <p className="mb-3 flex items-center gap-1.5 text-sm text-gray-500">
        <span>📍</span> {tournament.location_name}
      </p>
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50/50 via-white to-amber-50/30">
      ...
      <nav className="bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          <Link href="/redesign/2" className="flex items-center gap-2">
      ...
      <header className="px-5 pb-8 pt-12 text-center">
        <p className="mb-2 text-sm text-green-600">Hey there! 👋</p>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-16">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((t) => (
              <WarmCard key={t.id} tournament={t} />
            ))}
          </div>
        )}
      </section>
