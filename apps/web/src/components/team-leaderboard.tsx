...
function MemberRow({ m }: { m: LbMember }) {
  return (
    ...
    <div className="grid grid-cols-[1fr_auto_0.75rem] items-center gap-x-1.5">
      ...
      <span className={`text-right text-sm font-bold tabular-nums ${RATING_COLOR[m.status]}`}>
...
function TeamRow({ team, scoreLabel }: { team: LbTeam; scoreLabel: string }) {
  return (
    <div className="grid grid-cols-[22px_1fr_auto] items-center gap-x-3 border-b border-gray-100 px-3 py-2.5 last:border-b-0">
      <span className={`text-center text-sm font-bold tabular-nums ${team.rank != null ? "text-emerald-700" : "text-gray-300"}`}>
        {team.rank ?? "—"}
      </span>
      ...
      <div className="flex min-w-[56px] flex-col items-end">
        ...
        <span className={`text-xl font-extrabold leading-none tabular-nums tracking-tight ${team.verified ? "text-emerald-700" : "text-gray-500"}`}>
...
export function TeamLeaderboard({ event }: { event: TournamentEvent }) {
  ...
  if (lb.ranked.length === 0 && lb.awaiting.length === 0) {
    return <p className="py-2 text-sm text-gray-400">No player data available</p>;
  }
  ...
  return (
    <div className="mt-4 border-t border-gray-100 pt-3.5">
      ...
      {lb.ranked.length > 0 && (
        <>
          <p className="px-0.5 pb-1.5 pt-3 text-[10.5px] font-bold uppercase tracking-wide text-gray-400">
            Verified {groupNoun}{" "}
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-3 gap-y-1 px-0.5 text-[11px] text-gray-500">
